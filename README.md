# ApplyBuilder

[![scaladoc](https://javadoc.io/badge2/com.github.xuwei-k/applybuilder_3/javadoc.svg)](https://javadoc.io/doc/com.github.xuwei-k/applybuilder_3)
[![Maven Central](https://img.shields.io/maven-central/v/com.github.xuwei-k/applybuilder_3)](https://central.sonatype.com/artifact/com.github.xuwei-k/applybuilder_3)
[![Scala.js](https://www.scala-js.org/assets/badges/scalajs-1.4.0.svg)](https://www.scala-js.org)

```scala
libraryDependencies += "com.github.xuwei-k" %% "applybuilder" % "0.4.0"
```
